...
// Mirrors CoordinatorActivityDetail.jsx for a GRM Focal Person's own activity record.
export default function GrmActivityDetail({ activityId, onClose, onStatusChanged, canModerate }) {
  ...
  useEffect(() => {
    api.get(`/grm-activities/${activityId}`).then((res) => setData(res.data));
  }, [activityId]);

  async function setStatus(status) {
    setBusyAction(status === "flagged" ? "flag" : "verify");
    try {
      await api.patch(`/grm-activities/${activityId}/status`, { status, statusReason: reason });
      const res = await api.get(`/grm-activities/${activityId}`);
      setData(res.data);
